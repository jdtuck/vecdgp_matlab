...
report('U*U'' equals inv(Sigma) at m = n-1', err < 1e-8, sprintf('max err %.2e', err));

err = abs(2*sum(log(dU)) + log(det(K)));
report('log|Q| from diag(U)', err < 1e-8, sprintf('err %.2e', err));
...
report('Vecchia log-likelihood at m = n-1', abs(o.ll - ll_exact) < 1e-8, ...
       sprintf('%.10f vs %.10f', o.ll, ll_exact));

lls = zeros(1, 4); ms = [5 10 20 44];
for i = 1:4
    Am = vdgp_create_approx(X, ms(i), 'random', true);
    lls(i) = vdgp_logl(y, Am, theta, g, v, ct, false).ll;
end
mono = all(diff(abs(lls - ll_exact)) < 1e-8);
report('log-likelihood error decreases with m', mono, ...
...
report('profiled outer log-likelihood and tau2', ...
...
report('rand_mvn empirical covariance', Cerr < 0.06, sprintf('max err %.3f', Cerr));
...
report('lite prediction, m = n, matches exact', e1 < 1e-9 && e2 < 1e-9, ...
...
report('joint prediction, full m, matches exact', e1 < 1e-7 && e2 < 1e-6, ...
...
ll = vdgp_logl(1.0, A1, th, 1e-4, v, ct, false).ll;
for t = 1:T
...
report('MH kernel recovers the Gamma prior', em < 0.05 && es < 0.08, ...
       sprintf('mean %.3f (%.3f), sd %.3f (%.3f)', mean(keepT), mref, std(keepT), sref));
...
ll = vdgp_logl(1.0, wobj, 0.1, 1e-4, v, ct, true).ll;
for t = 1:T
    [w, wobj, ll] = vdgp_sample_w(1.0, w, Aw, wobj, 0.1, 0.1, 1e-4, ll, optsw);
    keepW(t) = w;
end
em = abs(mean(keepW)); es = abs(std(keepW) - 1);
report('ESS leaves the MVN prior invariant', em < 0.05 && es < 0.05, ...
...
for i = 2:200
    ...
    [~, o] = sort(dall);
    if ~isequal(sort(c(:)), sort(o(1:numel(c))))
        ok = false; break
    end
end
report('ordered nearest-neighbour sets are exact', ok, '');

% -- 9. maxmin ordering is a permutation ----------------------------------
Xm = rand(300, 3);
om = vdgp_order(Xm, 'maxmin');
report('maxmin ordering is a valid permutation', isequal(sort(om), 1:300), '');
...
report('two-layer Vecchia DGP on the deepgp example', rmse < 0.08 && cov95 > 0.85, ...
       sprintf('RMSE %.4f, coverage %.2f', rmse, cov95));

fprintf('\n%d passed, %d failed\n\n', np, nf);
if nf > 0, error('test_vdgp:fail', '%d checks failed.', nf); end
end
